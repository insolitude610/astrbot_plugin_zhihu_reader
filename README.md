...
知乎接口可能要求登录、触发风控或随站点更新而变化。评论请求因 Cookie 被拒绝时，插件会对公开评论自动进行一次匿名重试。付费文章回退只读取知乎页面中的结构化数据，不逆向或生成知乎的动态风控签名；如果页面要求交互式浏览器验证，插件会明确报告无法读取。开启登录态回退前，请确认 Bot 的触发范围可信，因为其他能发送链接的人可能间接读取该账号有权访问的文章。评论仍然失败时，正文会继续交给大模型，资料中会包含不可用原因，且失败结果不会作为正常内容缓存。可先用 `/知乎阅读 <url>` 检查当前链接、Cookie 和回退设置是否可用。
