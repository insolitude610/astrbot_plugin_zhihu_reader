...
- 支持从 Markdown/HTML 包装链接和带中文标点的消息中提取知乎链接，并去除重复链接。
- 自动读取现在也会检查 AstrBot 当前请求中的引用文本；命令参数可以使用包装后的单个知乎链接。
- 将空正文与非法 URL 区分处理，避免把知乎接口返回的空内容误报为链接格式错误。
- 显式声明 `socksio` 依赖，避免依赖预检查忽略 `httpx[socks]` extra 后在 SOCKS 代理环境启动失败。
